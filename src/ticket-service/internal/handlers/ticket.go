...
func (h *TicketHandler) CancelTicketHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.TicketRepo.UpdateTicket(&ticket); err != nil {
		log.Printf("Failed to create ticket: %s", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (h *TicketHandler) GetHealth(w http.ResponseWriter, r *http.Request) {

	w.WriteHeader(http.StatusOK)
}
func (h *TicketHandler) DeleteTicketHandler(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)

	if err := h.TicketRepo.DeleteTicket(params["id"]); err != nil {
		log.Printf("Failed to delete ticket: %s", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
